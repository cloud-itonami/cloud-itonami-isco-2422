(ns policy-administration.actor
  "PolicyAdministrationActor — the ISCO-08 2422 independent policy-
  administration actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  policy operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the PolicyAdvisor can never directly
  commit a record or dispatch a robot action the
  PolicyAdministrationGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [policy-administration.advisor :as advisor]
            [policy-administration.governor :as governor]
            [policy-administration.store :as store]))

(defn build-graph
  "Build a compiled PolicyAdministrationActor graph. `store`
  implements `policy-administration.store/Store`. `advisor` implements
  `policy-administration.advisor/Advisor` (defaults to
