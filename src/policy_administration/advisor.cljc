(ns policy-administration.advisor
  "PolicyAdvisor — proposes a policy-administration operation (draft a
  policy, publish a policy, publish a policy affecting client legal
  standing, disclose a confidential draft policy) for a registered
  ...
  provenance or legal-standing/disclosure risk;
  `policy-administration.governor` is the independent system that
  decides whether the proposal may proceed, per the itonami actor
  pattern.

  A proposal is a map:
    {:op :draft|:publish|:publish-affecting-legal-standing|:disclose-confidential-draft
...
(def ^:private system-prompt
  "You are a policy-administration advisor. Given a policy operation
