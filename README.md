...
- `src/policy_administration/store.kotoba` — `Store` protocol +
  `MemStore`: registered clients, committed records, an append-only
  audit ledger.
- `src/policy_administration/advisor.kotoba` — `Advisor` protocol;
  ...
  confidence).
- `src/policy_administration/governor.kotoba` —
  ...
  sign-off.
- `src/policy_administration/actor.kotoba` — `build-graph`,
